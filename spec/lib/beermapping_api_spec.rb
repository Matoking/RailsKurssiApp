...
describe "BeermappingApi" do
    describe "in case of cache miss" do
        ...
        it "When HTTP GET returns no entries, it is parsed and returned" do
            ...
            stub_request(:get, /.*absolutelynoplacelikethisexists/).to_return(body: canned, headers: { 'Content-Type' => "text/xml" })

            places = BeermappingApi.places_in("absolutelynoplacelikethisexists")

            expect(places.size).to eq(0)
        end

        it "When HTTP GET returns multiple entries, it is parsed and returned" do
            canned = <<-END_OF_STRING
            ...
            stub_request(:get, /.*helsinki/).to_return(body: canned, headers: { 'Content-Type' => "text/xml" })
